...
async function findPrivateSaleByToken(strapi, token) {
    ...
    for (const artist of artists) {
        ...
        for (const p of prints) {
            if ((p === null || p === void 0 ? void 0 : p.private) && (p === null || p === void 0 ? void 0 : p.privateToken) === token) {
...
exports.default = strapi_1.factories.createCoreController('api::artist.artist', ({ strapi }) => ({
    ...
    async getPrivateSaleByToken(ctx) {
        ...
        if (!found)
            return ctx.notFound('Vente privee introuvable ou lien expire');
    ...
    async createPrivateSaleCheckout(ctx) {
        ...
        if (p.allowCustomPrice) {
            ...
            if (clientAmount < basePrice) {
                return ctx.badRequest(`Le montant minimum pour cette oeuvre est de ${basePrice}$`);
            }
        ...
        try {
            ...
            return ctx.internalServerError((err === null || err === void 0 ? void 0 : err.message) || 'Erreur creation session de paiement');
        }
    },
    /**
    ...
    async publicCmsList(ctx) {
        try {
            ...
            strapi.log.error(`[publicCmsList] ${(err === null || err === void 0 ? void 0 : err.message) || err}`);
    ...
    async adminListAll(ctx) {
        ...
        try {
            ...
            for (const a of (artists || [])) {
                if (a.slug)
                    artistsBySlug[String(a.slug).toLowerCase()] = a;
                if (a.email)
                    artistsByEmail[String(a.email).toLowerCase()] = a;
            }
            ...
            for (const ur of (artistRoles || [])) {
                ...
                if (matchedBySlug || matchedByEmail)
                    continue;
                list.push({
                    documentId: null,
                    slug: ur.artistSlug || '',
    ...
    async adminGetDetail(ctx) {
        ...
        try {
            ...
            try {
                ...
                if (emailToQuery) {
                    withdrawals = await strapi.documents('api::withdrawal-request.withdrawal-request').findMany({
                        filters: { email: { $eqi: emailToQuery } },
                        sort: 'createdAt:desc',
                        limit: 50,
                    });
                }
            ...
            const paypalEmail = ((_a = withdrawals[0]) === null || _a === void 0 ? void 0 : _a.paypalEmail) || '';
            // ============================================================
            ...
            const itemsViews = [
                ...prints.map((p) => ({ id: p.id, category: 'prints', title: p.titleFr || p.titleEn || p.id, views: Number(p.views) || 0 })),
                ...stickers.map((s) => ({ id: s.id, category: 'stickers', title: s.titleFr || s.titleEn || s.id, views: Number(s.views) || 0 })),
            ];
            const stats = {
    ...
    async adminTopArtworks(ctx) {
        ...
        const { id } = ctx.params;
        if (!id)
            return ctx.badRequest('id ou slug requis');
        // Lazy import - evite de charger la lib GA4 (et ses ~40MB de deps
        // gRPC) si l'endpoint n'est jamais appele.
        const { topPagesForPrefix, getGAClient } = await Promise.resolve().then(() => __importStar(require('../../../utils/ga4')));
        ...
        if (!artist)
            return ctx.notFound(`Artiste ${id} introuvable`);
