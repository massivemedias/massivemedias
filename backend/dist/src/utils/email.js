...
function buildOrderDeliveredHtml(nom, orderRef) {
...
async function sendOrderDeliveredEmail(email, nom, orderRef) {
    const resend = getResend();
    if (!resend) {
...
function buildArtistWelcomeHtml(data) {
...
async function sendArtistWelcomeEmail(data) {
    const resend = getResend();
    if (!resend) {
    ...
    try {
        ...
        console.error('[email] Erreur envoi bienvenue artiste:', (err === null || err === void 0 ? void 0 : err.message) || err);
        return false;
    }
}
