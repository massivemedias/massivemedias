...
exports.dispatchWebhook = void 0;
...
async function dispatchWebhook(eventName, data) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            console.error(`[WEBHOOK] Failed to dispatch event=${eventName} status=${res.status} ${res.statusText}`);
        }
    }
    catch (error) {
        // Network error, abort timeout, DNS fail, etc. - tous absorbes ici.
        const reason = (error === null || error === void 0 ? void 0 : error.name) === 'AbortError'
