...
exports.shouldSendThrottledAlert = void 0;
async function shouldSendThrottledAlert(alertKey, throttleMinutes = 60) {
    try {
        const existing = await strapi.db
            .query('api::webhook-alert-throttle.webhook-alert-throttle')
            .findOne({ where: { alertKey } });
        const now = Date.now();
        const throttleMs = throttleMinutes * 60000;
        ...
        if (existing) {
            ...
            if (now - lastSent < throttleMs) {
                ...
                strapi.log.info(`[throttle] Alert '${alertKey}' suppressed (${secondsAgo}s since last, throttle=${throttleMinutes}min, count=${existing.sendCount || 0})`);
        ...
        strapi.log.warn(`[throttle] Check failed for '${alertKey}': ${(err === null || err === void 0 ? void 0 : err.message) || err} - sending anyway (fail-open)`);
        return true;
    }
}
exports.shouldSendThrottledAlert = shouldSendThrottledAlert;
