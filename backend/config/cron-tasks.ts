 ...
 */
import { sendPaymentReminderEmail, sendProductionDigestEmail } from '../src/utils/email'
  ...
  productionDigest: {
    task: async ({ strapi }: { strapi: any }) => {
      ...
      try {
        ...
        const rows = pending.slice(0, DIGEST_MAX_ROWS).map((order: any) => {
          const orderRef = (
        ...
        const ok = await sendProductionDigestEmail({ rows, overflowCount })
        ...
        strapi.log.error(`[CRON][productionDigest] FATAL: ${err?.message || err}`)
      }
    },
    options: {
    ...
    task: async ({ strapi }: { strapi: any }) => {
      ...
      try {
        ...
        for (const order of candidates) {
          ...
          const invoice = o.invoice as any;
          ...
          try {
            ...
            failed++;
            strapi.log.error(`[CRON][paymentReminder] echec envoi pour ${orderRef}: ${err?.message || err}`);
          }
        }
        ...
        strapi.log.error(`[CRON][paymentReminder] FATAL: ${err?.message || err}`);
      }
    },
    options: {
  ...
  visitorHitPurge: {
    task: async ({ strapi }: { strapi: any }) => {
      try {
        ...
        if (purges > 0) strapi.log.info(`[CRON][visitorHitPurge] ${purges} pageviews > 48h supprimes`);
      } catch (err: any) {
        strapi.log.error(`[CRON][visitorHitPurge] FATAL: ${err?.message || err}`);
      }
    },
    options: {
      rule: '17 * * * *',
