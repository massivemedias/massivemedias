...
export default factories.createCoreController('api::artist.artist', ({ strapi }) => ({
  ...
  async adminInitProfile(ctx) {
    ...
    try {
      ...
      const role = (roleEntries || [])[0] as any;
      ...
      let slug = String(body?.slug || role.artistSlug || '').trim().toLowerCase()
        .replace(/[^a-z0-9-]/g, '-')
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '');
      if (!slug) {
      ...
      const name = String(body?.name || role.displayName || email.split('@')[0] || 'Artiste').trim();
      ...
      if (!role.artistSlug || role.artistSlug !== slug) {
        try {
          await strapi.documents('api::user-role.user-role').update({
            ...
            data: { artistSlug: slug } as any,
          });
        } catch (linkErr: any) {
          strapi.log.warn(
            `[adminInitProfile] Echec link user-role.artistSlug pour ${email}: ${linkErr?.message || linkErr}`
          );
        }
      }
      ...
      strapi.log.error(`[adminInitProfile] Erreur sur ${email}: ${err?.message || err}\n${err?.stack || ''}`);
      ctx.throw(500, err?.message || 'Erreur initialisation profil');
    }
  },
  ...
  async adminGetDetail(ctx) {
    ...
    try {
      ...
      const resolveRate = (productId) => {
        ...
        if (pid.startsWith(`artist-sticker-pack-${slug}-`)) return stickerRate;
  ...
  async adminTopArtworks(ctx) {
    ...
    try {
      ...
      strapi.log.warn(`[adminTopArtworks] GA4 API error pour ${artistSlug}: ${err?.message || err}`);
      ctx.body = {
        success: true,
        topArtworks: [],
        status: 'api_error',
