...
import {
  ...
  // STICKERS-SHOP-B (8 juillet 2026) : collection Massive (unites + packs).
...
export default factories.createCoreController('api::order.order', ({ strapi }) => ({
  ...
  async createPaymentIntent(ctx) {
    ...
    const collectionUnitCount = items.reduce((sum: number, it: any) =>
      String(it?.productId || '').startsWith('sticker-massive-') ? sum + (Number(it?.quantity) || 1) : sum, 0)
    if (collectionUnitCount > 0 && collectionUnitCount < STICKER_COLLECTION_MIN_UNITS) {
      return ctx.badRequest(`Minimum ${STICKER_COLLECTION_MIN_UNITS} stickers de la collection par commande (panier actuel: ${collectionUnitCount})`)
    }
    ...
    let piCachedArtists: any[] | null = null;
    const piCachedProducts: Record<string, any | null> = {};
    const piSkuDeps = {
      getArtists: async () => {
      ...
      getProductBySlug: async (slug: string) => {
        if (!(slug in piCachedProducts)) {
          ...
          piCachedProducts[slug] = found && found.length > 0 ? (found[0] as any) : null;
        }
    ...
    let subtotal = 0;
    for (const item of items) {
      const resolution = await resolveSkuPrice(item, piSkuDeps);
      if (!resolution.ok) {
        // Loi 25 : IP hashee (sha256 sale via QR_IP_HASH_SALT), jamais en clair.
        strapi.log.warn(`[sku-registry] REFUS endpoint=payment-intent productId=${String((item as any)?.productId || '(vide)')} family=${resolution.family} email=${customerEmail || '?'} ipHash=${hashIpForLog(ctx.request.ip)} raison="${resolution.reject}"`);
  ...
  async createCheckoutSession(ctx) {
    ...
    const csCachedProducts: Record<string, any | null> = {};
    const csSkuDeps = {
      getArtists,
      getProductBySlug: async (slug: string) => {
        if (!(slug in csCachedProducts)) {
          const found = await strapi.documents('api::product.product').findMany({ filters: { slug } });
          csCachedProducts[slug] = found && found.length > 0 ? (found[0] as any) : null;
        }
    ...
    for (const item of items) {
      ...
      const resolution = await resolveSkuPrice(item, csSkuDeps);
      if (!resolution.ok) {
        // Loi 25 : IP hashee (sha256 sale via QR_IP_HASH_SALT), jamais en clair.
        strapi.log.warn(`[sku-registry] REFUS endpoint=checkout-session productId=${String((item as any)?.productId || '(vide)')} family=${resolution.family} email=${customerEmail || '?'} ipHash=${hashIpForLog(ctx.request.ip)} raison="${resolution.reject}"`);
        return ctx.badRequest(resolution.reject);
      }
  ...
  async generatePortfolio(ctx) {
    ...
    if (o.status !== 'delivered') {
      return ctx.badRequest(
        `Seules les commandes livrees peuvent generer un projet portfolio (status actuel: ${o.status}).`,
      );
    }
    ...
    const filesPayload = (ctx.request as any).files;
    let mediaIds: number[] = [];
    if (filesPayload && filesPayload.files) {
      ...
      try {
        ...
        mediaIds = (uploaded as any[]).map((m) => m.id).filter(Boolean);
        strapi.log.info(`[generatePortfolio] ${mediaIds.length} image(s) uploadees pour ${documentId}`);
      } catch (err: any) {
        strapi.log.error(`[generatePortfolio] echec upload : ${err?.message || err}`);
    ...
    const itemTypes = Array.from(
      new Set(
        items
          .map((i: any) => i?.productName || i?.name || i?.type || '')
    ...
    const itemLines = items
      .map((i: any) => {
        ...
        const finishPart = i?.finish ? ` (${i.finish})` : '';
    ...
    try {
      ...
      strapi.log.info(`[generatePortfolio] Project draft cree pour ${documentId} -> ${project?.documentId}`);
    } catch (err: any) {
      strapi.log.error(`[generatePortfolio] echec creation projet : ${err?.message || err}`);
  ...
  async crmClient(ctx) {
    ...
    const ltv = orders.reduce((sum, o: any) => {
      if (REVENUE_EXCLUDED.has(o.status)) return sum;
      return sum + (Number(o.total) || 0);
    }, 0);

    // Nom + companyName depuis la commande la plus recente (orders[0]
    // car sort: createdAt:desc).
    const mostRecent = orders[0] as any;
  ...
  async crmCreatePromo(ctx) {
    ...
    try {
      ...
      const tryCreatePromo = async (code: string) => {
      ...
      try {
        ...
        const isTaken = /already|taken|exist/i.test(err?.message || '');
      ...
      strapi.log.error(`[crmCreatePromo] ECHEC pour ${email}: ${err?.message || err}`);
      ctx.status = 500;
      ctx.body = {
        error: {
          status: 500,
          name: 'PromoCreationFailed',
          message: err?.message || 'Echec creation code promo Stripe',
          code: 'PROMO_FAILED',
        },
      };
    }
  ...
  async updateStatus(ctx) {
    ...
    if (sendEmail && newStatus === 'delivered' && (order as any).customerEmail) {
      ...
      const customerName = (order as any).customerName || customerEmail.split('@')[0];
      const orderData = updated as any;
      const sid = orderData.stripePaymentIntentId || orderData.documentId || '';
      const orderRef = orderData.orderRef || (order as any).orderRef || String(sid).slice(-8).toUpperCase();

      sendOrderDeliveredEmail(customerEmail, customerName, orderRef)
        .then(ok => {
          if (ok) strapi.log.info(`[updateStatus] Email "commande livree + avis Google" envoye a ${customerEmail} pour ${documentId}`);
        })
  ...
  async adminStats(ctx) {
    ...
    // archive.
    const overrideMonth = String((ctx.query as any)?.month || '').match(/^\d{4}-\d{2}$/)
      ? String((ctx.query as any).month)
      : null;
    const now = new Date();
    ...
    const startOfMonth = (d: Date) => new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), 1, 4, 0, 0)); // 04:00 UTC ~= 00:00 Montreal (EST/EDT differ - approximation conservative pour les bornes)
    ...
    const fetchBucket = async (start: Date, end: Date) => {
      const rows: Array<{ status: string; total_cents: string; count: string }> = await knex('orders')
        .select('status')
      ...
      const sumOf = (statuses: string[]) =>
        statuses.reduce((acc, s) => acc + (byStatus[s]?.total || 0), 0);
      return {
    ...
    const fetchLeadCount = async (start: Date, end: Date) => {
      const [row]: Array<{ count: string }> = await knex('contact_submissions')
        .count({ count: '*' })
        .where('created_at', '>=', start.toISOString())
        .andWhere('created_at', '<', end.toISOString());
    ...
    const [activeRow]: Array<{ count: string }> = await knex('orders')
      .count({ count: '*' })
