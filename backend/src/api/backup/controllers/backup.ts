...
function sanitize(records: any[]): any[] {
...
async function safeFindMany(uid: string): Promise<any[]> {
  try {
    const docs = await (strapi as any).documents(uid).findMany({});
    return Array.isArray(docs) ? docs : [];
  } catch (err: any) {
    (strapi as any)?.log?.warn?.(`[backup] safeFindMany ${uid} failed: ${err?.message || err}`);
    return [];
  }
}
...
export default {
   ...
   * }
   */
  async exportAll(ctx: any) {
    if (!(await requireAdminAuth(ctx))) return;

    const startedAt = Date.now();
    const adminEmail = (ctx.state as any)?.adminUserEmail
      || (ctx.state as any)?.adminAuthMethod
      || 'unknown-admin';
    ...
    try {
      ...
      (strapi as any)?.log?.info?.(
        `[backup] Export by ${adminEmail}: ${totalRecords} records in ${elapsedMs}ms `
      ...
      (strapi as any)?.log?.error?.(`[backup] Export failed for ${adminEmail}: ${err?.message || err}`);
      ctx.status = 500;
      ctx.body = {
        error: {
          status: 500,
