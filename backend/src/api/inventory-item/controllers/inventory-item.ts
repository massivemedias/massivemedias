...
const EXPLICIT_RECLASSIFY_PLAN: Array<{ pattern: string; target: string }> = [
 ...
 */
export async function runExplicitReclassificationApril2026(strapi: any): Promise<{
  scanned: number;
  reclassified: number;
  alreadyCorrect: number;
  unmatched: number;
  changes: Array<{ id: number; documentId: string; name: string; from: string; to: string }>;
  ...
  const changes: Array<{ id: number; documentId: string; name: string; from: string; to: string }> = [];
  ...
  for (const change of changes) {
    try {
      await strapi.documents('api::inventory-item.inventory-item').update({
        ...
        data: { category: change.to } as any,
      });
    } catch (err: any) {
      errors.push({
        documentId: change.documentId,
...
export default factories.createCoreController('api::inventory-item.inventory-item', ({ strapi }) => ({
  ...
  async createItem(ctx) {
    ...
    if (existingItems.length > 0) {
      ...
      else nextNum = existingItems.length + 1;
    }
  ...
  async reclassifyTaxonomy(ctx) {
    ...
    const items = await strapi.documents('api::inventory-item.inventory-item').findMany({
      filters: { category: { $in: SOURCE_CATEGORIES } as any } as any,
      limit: 500,
    }) as any[];

    // Heuristiques par mot-cle. Ordre : plus specifique en premier.
    function classify(nameFr: string, nameEn: string): string | null {
    ...
    for (const change of plan) {
      try {
        await strapi.documents('api::inventory-item.inventory-item').update({
          documentId: change.documentId,
          data: { category: change.newCategory } as any,
        });
        applied++;
      } catch (err: any) {
        errors.push({ documentId: change.documentId, sku: change.sku, error: err?.message });
      }
    }
  ...
  async reclassifyExplicitApril2026(ctx) {
    ...
    try {
      ...
      strapi.log.info(
        `[reclassifyExplicit] scanned=${report.scanned} reclassified=${report.reclassified} already=${report.alreadyCorrect} errors=${report.errors.length}`
      );
      ctx.body = { data: report };
    } catch (err: any) {
      strapi.log.error('[reclassifyExplicit] ECHEC :', err);
      return ctx.throw(500, err?.message || 'Erreur reclassification');
    }
  },
}));
