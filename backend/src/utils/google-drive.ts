 ...
 */
export async function getOrCreateFolder(token: string, parentFolderId: string, folderName: string): Promise<string> {
  ...
  if (!createRes.ok) throw new Error(`Failed to create folder '${folderName}': ${await createRes.text()}`);
