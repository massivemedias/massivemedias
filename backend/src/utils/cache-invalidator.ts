...
function buildArtistUrls(slug: string): string[] {
 ...
 */
async function purgeCloudflare(urls: string[], log: (msg: string) => void): Promise<PurgeResult> {
  ...
  try {
    ...
    if (!data?.success) {
      log(`[cache-invalidator] Cloudflare success=false: ${JSON.stringify(data?.errors || []).slice(0, 200)}`);
    ...
    log(`[cache-invalidator] Cloudflare fetch error: ${err?.message || err}`);
    return { ok: false, provider: 'cloudflare', urlsPurged: 0, error: err?.message || 'fetch failed' };
  }
}
 ...
 */
export async function invalidateArtistCache(
  slug: string,
  strapiLog?: { info: (m: string) => void; warn: (m: string) => void },
): Promise<PurgeResult> {
  const log = (msg: string) => {
    if (strapiLog?.info) strapiLog.info(msg);
